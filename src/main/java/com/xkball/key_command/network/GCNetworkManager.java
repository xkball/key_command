package com.xkball.key_command.network;
...
public enum GCNetworkManager {
    ...
    //当然也可以用别的，保证唯一即可。
    private final FMLEventChannel channel = NetworkRegistry.INSTANCE.newEventDrivenChannel(KeyCommand.MODID);
    ...
    private FMLProxyPacket createFMLProxyPacket(GCPacket pkt) {
        ...
        try {
            ...
            KeyCommand.logger.debug(e);
        }
        return new FMLProxyPacket(new PacketBuffer(buffer), KeyCommand.MODID);
    }
}
